...
// if everything is ok, try to upload file
    ...
    if($curr_password == $admin['password']){
        ...
        try{
            $stmt = $conn->prepare("SELECT COUNT(*) AS numrows FROM driver WHERE email=:email AND id <>:id");
            ...
            if($row['numrows'] > 0){
                ...
                $stmt = $conn->prepare("UPDATE driver SET email=:email, password=:password, firstname=:firstname, lastname=:lastname, mobile=:mobile,photo=:photo WHERE id=:id");
